...
void ADXL345::readAccel(int *x, int *y, int *z) {
  ...
  // each axis reading comes in 10 bit resolution, ie 2 chars.  Least Significat Byte first!!
...
void ADXL345::readFrom(char address, int num, char _buff[]) {
  ...
  Wire.beginTransmission(DEVICE); // start transmission to device
  Wire.requestFrom(DEVICE, num);    // request 6 chars from device

  int i = 0;
  while(Wire.available())         // device may send less than requested (abnormal)
  { 
    _buff[i] = Wire.receive();    // receive a char
    i++;
  }
  if(i != num){
